...
export default function ClienteFormPage({ params }: { params?: { id?: string } }) {
  ...
  const seleccionarSugerenciaDireccion = (sugerencia: DireccionSugerencia) => {
    const comuna = sugerencia.context?.find(
      c => c.id.startsWith('locality.') || c.id.startsWith('place.')
    )?.text || '';
    setFormData(prev => ({
      ...prev,
      direccion: sugerencia.place_name,
      ...(comuna ? { comuna } : {}),
    }));
    setMostrarSugerenciasDireccion(false);
    setSugerenciasDireccion([]);
        ...
        <div ref={direccionRef} className="relative">
          ...
          <div className="relative">
            <input
              ...
              className="w-full bg-slate-700 text-white rounded-lg px-4 py-2 focus:ring-2 focus:ring-primary pr-8"
